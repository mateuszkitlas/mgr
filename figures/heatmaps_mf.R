...
# # Root scores
data <- read.csv("results/table_roots_mf.csv")
...
scale_x_discrete(labels = c("mf" = "OC-MF-Score", "ra" = "RAscore", "sa" =  "SAscore", "sc" = "SCScore", "syba" = "SYBA") ) +
...
ggsave("figures/heatmap_root_scores_mf.pdf",
       width = 5, height = 2.5)
...
data <- read.csv("results/table_siglings_mf.csv")
...
a = ggplot(data, aes(x=X0, y=X1, fill=X2)) +
  ...
  scale_y_discrete(breaks = custom_order,
                   labels = str_wrap(c("sa min" = "SAscore maximum",# Warning, min max swapped
                                       ...
                                       "syba min" = "SYBA minimum",
                                       "syba avg" = "SYBA mean",
                                       "mf max" = "OC-MF-Score maximum",
...
# # Parent-child scores
data <- read.csv("results/table_parental_mf.csv")
...
t# # Heatmap of AUC
library(dplyr)
data.in <- read.csv("results/table_aucs_mf.csv")
