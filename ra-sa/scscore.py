...
def linear(input_, output_size, scope, reuse=False, init_bias=0.0):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        W = tf.get_variable("Matrix", [
                            shape[-1], output_size], tf.float32, tf.random_normal_initializer(stddev=stddev))
...
class SCScorer():
    ...
    def get_score_from_smi(self, smi='', v=False):
        ...
        while len(src_batch) != (batch_size * 2):  # round out last batch
            src_batch.append('')
        src_mols = np.array(
            list(map(self.smi_to_fp, src_batch)), dtype=np.float32)
    ...
    def dump_to_numpy_arrays(self, dump_path):
        ...
        with open(dump_path, 'wb') as fid:
            pickle.dump([v.eval(session=self.session)
                        for v in tf.trainable_variables()], fid, -1)


def get_sc_scorer() -> Callable[[str], float]:
    model = SCScorer()
    model.build()
    model.restore(os.path.join(project_dir, 'data', 'sc_models',
                  'full_reaxys_model_1024bool'), 'ckpt-10654')
